...
const PredictionView = ({ data }: PredictionViewProps) => {
          ...
          <ul className="flex flex-col gap-4">
            {face
              ...
              .map((person, index) => (
                <>
                  <li key={index}>
                    <Badge className="mb-4">P{person.id}</Badge>
    ...
    <ul className="flex flex-col gap-4">
      {emotions?.map((emotion) => (
        <li className="h-full w-full">
          ...
          <span className="text-stone-400">{emotion.score.toFixed(2)}</span>
          <Progress.Root
          ...
          >
            <Progress.Indicator
              ...
              style={{
                transform: `translateX(-${100 - emotion.score * 100}%)`,
              }}
            />
          </Progress.Root>
        </li>
